...
from .forms import UploadResultsFile
...
def save_student_result(request):
    ...
    form=UploadResultsFile(request.POST, request.FILES)
    student_admin_id=request.POST.get('student_list')
    assignment_marks=request.POST.get('assignment_marks')
    exam_marks=request.POST.get('exam_marks')
    #exam_file=request.POST.get('exam_file')
    subject_id=request.POST.get('subject')

    exam_file=request.FILES['exam_file']
    fs=FileSystemStorage()
    filename=fs.save(exam_file.name, exam_file)
    exam_file_url=fs.url(filename)
    ...
    try:
        ...
        if check_exist:
            ...
            result.subject_result_file=exam_file_url
            result.save()
            messages.success(request, "Successfully Updated Result")
            return HttpResponseRedirect(reverse("staff_add_result"))
        else:
            result=StudentResult(student_id=student_obj,subject_id=subject_obj,subject_exam_marks=exam_marks,subject_assignment_marks=assignment_marks,subject_result_file=exam_file)
            result.save()
